...
def compare_faces(embedding1, embedding2, threshold=0.8):  # Default threshold is 0.8
    if embedding1 is None or embedding2 is None:
        return None, False  # Handle cases where embeddings are not available

    distance = np.linalg.norm(embedding1 - embedding2)
    return distance, distance < threshold  # Return True if faces match (distance < threshold)

def verification(request):
    if request.method == 'POST':
        try:
            # Get the captured photo from the POST request
            captured_photo = request.FILES.get('captured_image')
            if not captured_photo:
                return JsonResponse({'error': 'Captured photo not found.'}, status=400)

            # Get the uploaded image path
            uploaded_image_path = request.POST.get('uploaded_image_path')
            if not uploaded_image_path:
                return JsonResponse({'error': 'Uploaded image path not received.'}, status=400)

            # Construct the full server-side path for the original photo
            original_photo_path = os.path.join(settings.MEDIA_ROOT, uploaded_image_path.replace(settings.MEDIA_URL, '', 1))
            if not os.path.exists(original_photo_path):
                return JsonResponse({'error': 'Original photo not found on server.'}, status=400)

            try:
                # Load the images
                original_photo = Image.open(original_photo_path).convert("RGB")
                captured_image = Image.open(captured_photo).convert("RGB")

                # Get embeddings for both images
                embedding1 = get_embedding(original_photo)
                embedding2 = get_embedding(captured_image)

                if embedding1 is None or embedding2 is None:
                    return JsonResponse({'error': 'Could not detect face in one or both images.'}, status=400)

                # Compare faces
                distance, match = compare_faces(embedding1, embedding2)

            except Exception as e:
                logger.exception(f"Error during face comparison: {e}")
                return JsonResponse({'error': 'An error occurred during face comparison.'}, status=500)

            # Handle face matching logic
            if match:
                # Print a message when faces match
                logger.info(f"Faces match with a distance of {distance:.2f}. Redirecting to the question page.")

                # Retrieve interview data and session information
                interview_type = request.session.get('interview_type')
                resume_text = request.session.get('resume_text')

                if not interview_type or not resume_text:
                    return JsonResponse({'error': 'Interview setup incomplete. Please restart.'}, status=500)

                # Generate interview data and store in session
                ...
                return JsonResponse({'redirect_url': reverse('question', kwargs={'interview_type': interview_type})})

            else:
                return JsonResponse({'error': f'Faces do not match (distance: {distance:.2f}). Please try again.'}, status=400)

        except Exception as e:
            logger.exception(f"Error in verification view: {e}")
            return JsonResponse({'error': 'An unexpected error occurred during verification.'}, status=500)

    else:  # Handle GET request
        candidate_image_path = request.session.get('candidate_image_path')
        ...
        return render(request, 'hr_interview/verification.html', {'candidate_image_path': candidate_image_path})
...
def index(request, interview_type=None):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                resume_file = form.cleaned_data['resume_image']
                candidate_image = request.FILES.get('candidate_image')

                if not candidate_image:
                    messages.error(request, "Please upload your photo.")
                    return redirect('index', interview_type=interview_type)

                # Generate UUID filenames
                resume_filename = f"resume_{uuid.uuid4()}.{resume_file.name.split('.')[-1]}"
                candidate_filename = f"photo_{uuid.uuid4()}.{candidate_image.name.split('.')[-1]}"


                resume_image_path = os.path.join(settings.MEDIA_ROOT, resume_filename)
                candidate_image_path = os.path.join(settings.MEDIA_ROOT, candidate_filename)

                # Save files
                with open(resume_image_path, 'wb') as f:
                    for chunk in resume_file.chunks():
                        f.write(chunk)

                with open(candidate_image_path, 'wb') as f:
                    for chunk in candidate_image.chunks():
                        f.write(chunk)


                resume_text = extract_text_from_image(resume_image_path)
                if not resume_text:
                    # Clean up uploaded files if extraction fails
                    os.remove(resume_image_path)
                    os.remove(candidate_image_path)
                ...
                request.session['candidate_image_path'] = settings.MEDIA_URL + candidate_filename  # Relative URL
                ...
                return redirect('index', interview_type=interview_type)  # Redirect back to index with the interview_type


        else:
            # Handle invalid form submission (e.g., incorrect file type)
            messages.error(request, "Invalid form submission.") # Or more specific errors
            for field, errors in form.errors.items():
                for error in errors:
                    messages.error(request, f"{field}: {error}") # Display individual field errors
